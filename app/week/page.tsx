...
import { useSelectedDate, useSchedule, useSettings, useDatesWithNotes } from '@/hooks';
...
import { getWeekDates, getWeekday, formatDateDisplay, addDays } from '@/lib/utils/date';
...
export default function WeekPage() {
  ...
  const weekRange = `${formatDateDisplay(weekDates[0]).split(',')[1]?.trim() || ''} - ${formatDateDisplay(weekDates[6]).split(',')[1]?.trim() || ''}`;
    ...
    <>
      ...
      <main className="max-w-lg mx-auto">
        <div className="px-4 py-3 border-b border-surface-200 bg-white">
          <h2 className="font-medium text-gray-900">{formatDateDisplay(selectedDate)}</h2>
        </div>
