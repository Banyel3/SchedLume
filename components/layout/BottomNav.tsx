        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center justify-center min-w-16 min-h-12 px-3 py-2 rounded-xl transition-all duration-200 ${
                isActive
                  ? "text-primary-400 bg-primary-50"
                  : "text-gray-400 hover:text-gray-600 hover:bg-surface-100 active:scale-95"
              }`}
            >
              {item.icon}
