...
import { getSurroundingDates, getWeekdayShort, isToday, parseDate } from '@/lib/utils/date';
...
export function DateStrip({ selectedDate, onDateSelect, datesWithNotes = new Set(), range = 3 }: DateStripProps) {
  const dates = useMemo(() => getSurroundingDates(selectedDate, range), [selectedDate, range]);
  ...
  return (
    <div className="bg-white border-b border-surface-200">
      <div className="flex items-center justify-between px-2 py-3 max-w-lg mx-auto">
        ...
        <div className="flex items-center gap-1 overflow-x-auto scrollbar-hide">
          {dates.map((date) => {
            ...
            return (
              ...
              >
                <span className={`text-xs ${isSelected ? 'text-white/80' : 'text-gray-400'}`}>{dayName}</span>
